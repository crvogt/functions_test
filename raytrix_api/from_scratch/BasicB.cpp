...
int main(int argc, char* argv[])
{
	try
	{
		...
		Rx::CRxString sxFile = "C:\\Users\\carson\\Desktop\\gray_to_png\\5_ray.ray";
		...
		Rx::CRxImage xRawImage, xRawBayer;
		xImageFile.Read(&xRawImage, sxRawPngFile);
		xRawBayer.Create(xRawImage.GetFormat().m_iWidth, xRawImage.GetFormat().m_iHeight, Rx::Interop::Runtime28::EPixelType::BayerGB, Rx::Interop::Runtime28::EDataType::UByte, xRawImage.GetDataPtr());
		xCudaCompute.UploadRawImage(xRawBayer);
		xRawBayer.Destroy();
		...
		//xCudaCompute.GetParams().SetValue();
		double enableDem = 10;
		unsigned int enableDemMain = 1;
		xCudaCompute.GetParams().SetValue(Rx::LFR::Params::ECudaCompute::PreProc_Demosaic_Enable, enableDemMain);
		xCudaCompute.GetParams().SetValue(Rx::LFR::Params::ECudaCompute::PreProc_Demosaic_B, enableDem);
		xCudaCompute.GetParams().SetValue(Rx::LFR::Params::ECudaCompute::PreProc_Demosaic_G, enableDem);
		xCudaCompute.GetParams().SetValue(Rx::LFR::Params::ECudaCompute::PreProc_Demosaic_R, enableDem);
