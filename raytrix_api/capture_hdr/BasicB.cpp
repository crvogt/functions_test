...
int main(int argc, char* argv[])
{
	try
	{
		...
		xCamera.SetProperty(Rx::Interop::Runtime30::Camera::EProperty::Exposure, 1.0f);
		...
		std::cout << "Creating fibonacci bracket\n";
		float T = 1000.0; //ms
		float exposure = 3.0;
		...
		////Try to purge the memory
		//std::cout << "\nMemory purge test...\n";
		//for (int i = 0; i < 2; i++) {
		//	// Set the camera exposure in milliseconds	
		//	std::cout << "\nSetting exposure to: " << 3.0 << "\n";
		//	xCamera.SetProperty(Rx::Interop::Runtime30::Camera::EProperty::Exposure, 3.0f);
		...
		std::cout << "\nCapturing HDR set\n";
		// Prepare saving variables
		Rx::FileIO::CImage writeImg;
		std::string writeBase = "E:\\hdr\\fibonacci_1\\";
		
		counter = 0;
		for(int iter = 0; iter < exposure_vec.size(); iter++){

			 //Set the camera exposure in milliseconds	
			std::cout << "\nSetting exposure to: " << exposure_vec[iter] << "\n";
			xCamera.SetProperty(Rx::Interop::Runtime30::Camera::EProperty::Exposure, exposure_vec[iter]);
			//Rx::CRxString valu;
			float val;
			xCamera.GetProperty(Rx::Interop::Runtime30::Camera::EProperty::Exposure, val);
			std::cout << "Retrieved exposure: " << val << std::endl;
			using namespace std::this_thread; // sleep_for, sleep_until
			using namespace std::chrono; // nanoseconds, system_clock, seconds

			// ...actually wanted milliseconds... but whatevs
			sleep_for(microseconds(300000));
			...
			// Then save
			Rx::CRxString writeOut = "E:\\hdr\\fibonacci\\1\\raw\\";
			writeOut += counter;
			writeOut += ".png";
			//writeBase + std::to_string(counter) + ".png";
